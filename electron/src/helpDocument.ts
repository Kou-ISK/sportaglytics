...
const sections: HelpSection[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      'インスタンスの左右端を Command+Option（WindowsではCtrl+Alt）を押しながらドラッグすると、未選択でも開始・終了位置を調整できます。既存の選択は変更しません。修飾キーを押したままマウスを離すと確定し、キーを先に離すかEscで取り消します。同じ修飾キーで行内の赤い線からドラッグすると新しい区間を作成できます。区間の編集・作成中は再生位置を移動しません。',
      '赤い再生位置バーは上部のつまみをドラッグして動かします。目盛り・行内の線・アクションの通常クリックではシークしません。つまみの左右キー・Home/End、右クリックメニューのジャンプ、再生ツールバーでも位置を操作できます。',
