import {
  primitiveShape,
  primitiveSpacing,
} from '../../src/design-system/tokens/primitive';
...
const sections: HelpSection[] = [
  {
    ...
    steps: [
      '起動画面の「最近開いたパッケージ」から作業を再開できます。「履歴を検索」で名前・チーム・保存場所を絞り込み、行をクリックまたはTabで選んでEnterを押してください。行末の×は履歴だけを除き、ファイルを削除しません。',
      '読み込み中は開く操作を待ってください。失敗した場合は画面に残るエラーの詳細を確認できます。保存場所や外付けドライブを確認して「もう一度開く」で再試行するか、別の保存先を選び直してください。',
      '別の既存パッケージは「パッケージを開く」または ファイル > 開く > パッケージ… から選択できます。.stpkg をホーム画面へドラッグ&ドロップして開くこともできます。',
      '新しい分析を始める場合は「新しいパッケージを作成」または ファイル > 新規 > パッケージ… から、映像・アングル・チーム名を登録します。映像ファイルを起動画面へ直接ドロップする操作には対応しません。',
    ],
  },
  {
    id: 'playback',
    ...
    steps: [
      ...
      '手動同期モード（Cmd/Ctrl+Shift+T）で数字キーでアングルを表示し、独立タイムラインの赤いつまみで同じ瞬間に合わせて「同期点を設定」して「アングルを同期」を押します。数字キーでアングルを切り替え、同じキーまたは0で全表示へ戻ります。Spaceで再生/停止、左右キーでローカル映像の実フレームをコマ送り、Shift+左右で1秒移動、Sで同期点を設定します。後半でも同じ手順を繰り返し、「保存して閉じる」でPlaylistと書き出しへ反映します。',
      '「今の位置で同期」は既定で Cmd/Ctrl+Shift+M です。同期画面では表示中のアングルに同期点を設定します。設定 > ホットキー で現在の割り当てを確認・変更できます。',
    ],
  },
  {
    id: 'tagging',
    ...
    steps: [
      ...
      '編集モードではボタンの位置・サイズ・文字サイズ・色・ホットキー・リンクを変更できます。ディアクティベートリンクは元のボタンの記録開始時にだけ発火し、元のボタンの記録終了時には相手を停止しません。編集モード中は通常のコーディング用ホットキーを無効化します。',
    ],
  },
  {
    id: 'timeline',
    ...
    steps: [
      'ウィンドウ > タイムラインを開く で独立タイムラインを表示します。下部フッター左の「＋」で行を追加し、右側の「− 100% ＋」で表示倍率を変更できます。ピンチ、またはCmd/Ctrlを押しながらホイール操作でも、ポインター位置を中心に最大100倍まで拡大できます。',
      'インスタンスをクリックすると選択し、ドラッグすると別の行へ移動できます。Option/Altを押しながらドラッグするとコピーします。複数選択した状態では選択中のインスタンスをまとめて移動・コピーできます。',
      'アクションのない場所をクリックすると選択と枠を解除します。空いている範囲をドラッグすると範囲選択できます。インスタンスを右クリックすると、編集・削除・複製・ジャンプ・プレイリスト追加などの操作を開けます。Delete/Backspaceでは選択中のインスタンスをまとめて削除し、Cmd/Ctrl+Zで戻せます。Enterまたはダブルクリックで編集できます。',
      'Cmd/Ctrl+Aで全インスタンス、行が選択中ならその行内だけを選択します。行の右クリック「行内のインスタンスを選択」でも、行を残して一括削除・コピーできます。Escapeで選択を解除します。入力欄やダイアログでは文字編集とTab移動を優先します。',
      '1件を選び区間内に再生位置を合わせ、右クリック「再生位置で分割」またはCmd/Ctrl+Shift+Kで2分割します。同じ行の2件以上は「選択を1区間に結合」またはCmd/Ctrl+Shift+Jでまとめます。結合は間の空白を含み、ラベル・メモを保持します。いずれもCmd/Ctrl+Zを1回で取り消せます。',
      '行ヘッダーをCmd/Ctrlクリックすると複数の行を選択できます。行のDelete/Backspaceは確認後に所属インスタンスごと削除します。行の削除は取り消せません。',
      'インスタンスの左右端を Command+Option（WindowsではCtrl+Alt）を押しながらドラッグすると、未選択でも開始・終了位置を調整できます。既存の選択は変更しません。マウスを離すと確定します。途中でキーを先に離しても操作は継続し、Escまたはウィンドウからフォーカスを外すと取り消します。同じ修飾キーで行内の赤い線からドラッグすると新しい区間を作成できます。端の調整中は再生位置が追従し、新しい区間の作成中は再生位置を維持します。',
      '赤い再生位置バーは最上段の目盛りクリック・ドラッグ、またはつまみのドラッグで動かします。行内の通常クリックではシークしません。Cmd/Ctrl + Option/Altでアクションの端を調整すると映像もその位置へ追従します。つまみの左右キー・Home/End、右クリックメニューのジャンプ、再生ツールバーでも位置を操作できます。',
  ...
  },
  {
    ...
    steps: [
      ...
      '映像クリップは ファイル > エクスポート > 映像クリップ（オーバーレイ付き）から書き出します。対象パッケージのタイムラインに設定を開き、閉じている場合は再表示します。プレイリストを操作中なら、そのプレイリストの設定を開きます。書き出し中は専用進捗ウィンドウで進行状況を確認できます。',
      'PlaylistのPaintも映像に合成されます。各アングルの描画、追尾、フリーズと芝色処理を保持し、フリーズ中は音声を無音にします。2画面は縦横比を保って高さを揃えます。',
      'エンコード前に必要なローカル映像・元クリップと保存先を確認します。欠落があれば接続やファイルの場所を直して再試行してください。既存と同名の動画は連番で保存します。空き容量や開始後のドライブ切断は別途確認が必要です。',
  ...
  {
    id: 'paint',
    title: 'Paintで描画・追尾・戦術盤',
    summary: '選手を示す図形と、時間ごとの位置を編集する',
    steps: [
      'プレイリストでクリップを選んでPaintへ切り替えます。左のツールで描画し、右ペインのスタイル・動き・ピッチ・素材で調整します。右ペインは折りたためます。選手リンクは選手を順にクリックし、2〜15人でEnterまたは「リンクを確定」で確定します。',
      '図形を選択すると下部タイムラインに位置キーが表示されます。点をクリックしてその時刻へ移動し、映像上で位置を直します。点の横ドラッグで時刻を変更でき、拡大して近い点を選べます。開始位置のキーは時刻変更・削除できません。',
      'Delete / Backspaceは、位置キーを選択していればその点、図形だけを選択していれば図形を削除します。Paint中はクリップ削除を抑止します。文字や数値の入力中は通常どおり文字を消します。Cmd/Ctrl+Zで取り消せます。',
      '自動追尾では「動き」から対象範囲を指定し、選手の上半身の輪郭や識別できる模様を囲んで実行します。完全に追えた結果は自動反映され、途中結果は確認して採用します。追尾は映像の模様を利用するため、遮蔽・画面外・似た選手への重なりでは手動補正が必要です。',
      'ピッチの「4点で較正」で全面・ハーフなどの範囲を選び、俯瞰図と映像の番号を合わせます。別フレームでは線の一致を再確認します。「戦術盤を開く」で選手・ボール・矢印を配置し、映像から端末内で配置候補を認識できます。候補のチームと足元位置は手動で確認してください。戦術盤はPNG画像にでき、確定後にPlaylistを保存すると再利用できます。',
      'ディスクなどを足元へ置くと芝色処理が有効になり、芝以外を手前に残します。人物認識ではないため、緑色のユニフォームや芝以外の床では調整が必要です。「プレゼン」は提示に使うツールを絞った表示です。',
    ],
  },
  {
    id: 'settings',
