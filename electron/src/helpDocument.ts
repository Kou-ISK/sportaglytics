...
const sections: HelpSection[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      '手動同期モードでは左右の映像を独立して再生・シークできます。映像を選んでSpaceで再生/停止、左右キーでコマ送り、Shift+左右で1秒移動します。配置タイムラインで対象クリップをドラッグでき、連動プレビューで同期を確認できます。前半・後半が別ファイルの場合はA/C、B/Dなどの組合せごとに同じ場面を合わせ、「この位置で配置」から「同期を適用」を押します。Playlistと書き出しにも配置と同期補正が反映されます。同期メニューでは、自動同期（Cmd/Ctrl+Shift+S）、手動同期モード（Cmd/Ctrl+Shift+T）、同期オフセットのリセット（Cmd/Ctrl+Shift+R）を実行できます。',
