...
- **TimelineData.labels**: オプショナルなラベル配列を追加
  - 現行モデルは `labels` 中心
  - 旧 `actionType` / `actionResult` は読み込み時に `Type` / `Result` へ移行
...
1. **`convertToSCTimeline(timelineData, packagePath?)`**
   ...
   - labels配列から SCLabel を生成
   - 各行に一貫性のある色を自動割り当て

2. **`convertFromSCTimeline(scTimeline)`**
   - SCTimelineFile → TimelineData[] への変換
   - SCLabel を labels 配列として保持
   - 旧 `actionType` / `actionResult` グループは `Type` / `Result` へ移行
   - startTime順に自動ソート

3. **`normalizeTimelineData(data)`**
   - 古い形式のTimelineDataに labels配列を追加
   - 旧 actionType/actionResultから Type/Result ラベルを自動生成
...
| `SCInstance.notes`                  | `memo`                  |
| `SCLabel { group: "actionType" }`   | `labels[]` の `Type`    |
| `SCLabel { group: "actionResult" }` | `labels[]` の `Result`  |
...
1. **古い形式（labels なし）**
   ...
   → `labels` 配列を `Type` / `Result` として自動生成:

   ```json
   {
     "labels": [
       { "name": "Short", "group": "Type" },
       { "name": "Won", "group": "Result" }
     ]
   }
   ```
...
2. **新しい形式（labels あり）**
   ...
   ```
   → そのまま使用。旧 `actionType` / `actionResult` グループは読み込み時に `Type` / `Result` へ移行する

### 書き込み時

- `labels` 配列のみを書き込む
- 新規データでは `actionType` / `actionResult` グループを採用しない
