...
- 編集モード中のキャンバスサイズは、ユーザーがリサイズしたコードウィンドウ内のキャンバス領域に追従する。保存時は現在の runtime layout を `.stcw` へ保存し、メイン動画ウィンドウ側の実行 layout と同期する。
...
3. 右側のボタン設定でアクション / ラベル、色、表示名、hotkey、hotkey 表示有無を編集する
4. コードウィンドウをリサイズすると、編集キャンバスも利用可能領域に合わせてリサイズされる
