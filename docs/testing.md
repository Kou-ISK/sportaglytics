...
メニューバーの「新規コードウィンドウ…」は、実Electronで空の `.stcw` 作成、独立コードウィンドウの表示、編集モードへの遷移を検証します。設定画面を経由しないこと、「別名保存」1回につきnative save dialogが1回だけ開くこと、設定サブウィンドウでもsandbox preload経由で設定を読み込めることも検証対象です。
