...
720pの模擬入力で録画画面と映像ウィンドウを隠し、Code Windowのクリック/ホットキーを混用して25秒間連続記録します。区間をまたいでもvideo要素が同一で、emptied・waitingが発生せず映像時刻が進むことを確認します。可変フレーム時刻の区間重複は保存時に補正し、書き出しと再オープンも検査します。
