...
メニューバーのドキュメント操作は、実Electronで「ファイル > 新規 / 開く」の構造、重複する「コードウィンドウを開く」が存在しないこと、`Command+N`相当のパッケージ作成ウィザード表示を検証します。コードウィンドウは空の `.stcw` 作成、選択した `.stcw` の表示、編集モードへの遷移を確認します。設定画面を経由しないこと、「別名保存」1回につきnative save dialogが1回だけ開くこと、設定サブウィンドウでもsandbox preload経由で設定を読み込めることも検証対象です。
