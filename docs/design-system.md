...
5. **直接編集と実行表示の一致**
   - コードウィンドウのように実行対象を直接編集するUIは、実行・編集で同一のprops-only surfaceを共有する。
   - 編集選択、drag cursor、handleはoverlayとして追加し、保存対象の色、文字、輪郭、位置、寸法を編集専用表現で置き換えない。
   - `CodeWindowButtonSurface` は `src/components/ui/composites/` に置き、Electron APIやfeature stateへ依存させない。
