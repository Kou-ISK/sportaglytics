...
- `.stpl` - プレイリストファイル（パッケージ形式、内部に `playlist.json` と `videos/` を含む）
...
- macOSではパッケージ（ディレクトリ）として扱われ、Finderでは単一ファイルアイコンとして表示される
- 右クリック→「パッケージの内容を表示」で内部の `playlist.json` をテキストエディタで閲覧・編集可能
- ダブルクリックでSporTagLyticsが起動し、プレイリストウィンドウが開く
- **スタンドアロン形式**: `videos/` フォルダに映像ファイルを内包し、どこでも再生可能な自己完結型
- **参照形式**: 外部の映像パスを参照する軽量版（元の映像ファイルが必要）

**`.stcw` - コードウィンドウファイル**

- タグ付けボタンのレイアウト設定を保存
- ダブルクリックでSporTagLyticsの独立コードウィンドウが開き、レイアウトが自動的に読み込まれる
- JSON形式（version 1）
