...
- **分析ダッシュボード** (v0.4.0以降): `AnalysisPanelView`。共通カードレイアウトは `AnalysisCard`。Breakdown は `useActionBreakdown`、Matrix 軸設定は `useMatrixAxes`、フィルタは `useMatrixFilters`。独立ウィンドウ（`AnalysisWindowApp`）で表示され、メインウィンドウとの双方向同期を実現。軸設定とフィルター表示には `FilterSummaryBar` を使用（Chip + Popover パターンで省スペース化）。
...
- **分析UI**: カードは `AnalysisCard` を再利用。Breakdown データ整形は `useActionBreakdown`、Matrix 軸は `useMatrixAxes`、フィルタは `useMatrixFilters`。軸設定・フィルター表示は `FilterSummaryBar` を使用（コンパクトなChip + Popoverパターン）。
