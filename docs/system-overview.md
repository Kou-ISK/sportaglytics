...
- clip export の実行進捗は `electron/src/exportProgressWindow.ts` と `src/types/ipc/exportProgressWindow.ts` を境界にし、main 側の FFmpeg 実行ループから専用進捗ウィンドウへ送信する。playlist / timeline 側は `src/shared/clipExport/clipExportGateway.ts` 経由でウィンドウ起動と初期状態送信だけを行う
