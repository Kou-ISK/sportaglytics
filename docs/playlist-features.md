...
SporTagLyticsのプレイリスト機能は、タイムライン上の選択したイベントをプレイリストとしてまとめ、専用ウィンドウで連続再生・分析・クリップ書き出しを行える機能です。SportsCode準拠の設計で、フリーズフレーム・簡易描画・メモ編集など、詳細な分析を支援する機能を備えています。
...
| ------------------ | ----------- | ------------------------------- | ------------------ | -------------------- |
| **スタンドアロン** | `embedded`  | 映像ファイルを `videos/` に内包 | 大容量（映像込み） | 高い（自己完結型）   |
