...
- **要件**: 最大8アングルを同時に再生
- **複数クリップ**: 各アングル最大16本。クリップ前の空白秒数を指定し、空白区間は黒画面・無音で正規化
- **YouTube**: 各アングルに1 URLを割り当て可能。ローカル映像との混在シーケンスと複数URLの連結は対象外
- **登録操作**: アングル一覧、選択アングルの時系列クリップ、Inspector を分離して表示。ローカル映像の複数選択、ドラッグおよびボタンによる順序変更に対応
- **対応フォーマット**: MP4、MOV
