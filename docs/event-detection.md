...
追加された候補は通常Timelineの操作で確認します。同じ行の次候補は`Tab` / `Option/Alt+↓`、前候補は`Shift+Tab` / `Option/Alt+↑`で選択して再生位置へ移動できます。1つのプレーが複数区間になった場合は同じ行で複数選択して結合し、長い区間に複数プレーが含まれる場合は再生位置で分割します。いずれも1回のUndoで戻せます。詳細は[区間編集の操作](user-guide.md#ビジュアルタイムライン)を参照してください。

検出由来・確認済み状態は新たに保存しておらず、未確認候補の専用フィルタはありません。この修正を学習データとして外部送信する機能もありません。
...
`分析 > 自動イベント検出…` は、操作中のパッケージに属する独立ウィンドウを開きます。設定・進捗・結果をここへまとめ、映像ウィンドウのサイズや縦横比は変更しません。モデルの詳細評価は「モデルの評価・注意点」から展開します。狭い画面では設定部分だけがスクロールし、進捗と実行・取消操作は下部に残ります。

解析中も映像とTimelineを操作できます。「バックグラウンドで続行」またはOSの閉じる操作では解析画面を隠し、実行を継続します。同じメニューから再表示すると設定・進捗・結果を復元します。「キャンセル」は処理を取消します。映像ウィンドウを閉じる、または対象の映像構成を変更すると実行を取消し、古い結果を追加しません。

結果を追加する直前の最新Timelineで重複判定するため、解析中の手動追加を考慮します。解析画面を複数パッケージで開いた場合も、操作と結果を各所有元に限定します。所有関係とIPCの判断は[ADR 0038](adr/0038-detached-event-detection-window.md)を参照してください。
...
モデル一覧と初期設定は解析画面を最初に開く時に読み込みます。開いている間の映像メタデータ・コードウィンドウ更新では再読み込みせず、編集中のしきい値や追加先を保持します。解析対象アングルがなくなった場合だけ、利用可能なアングルへ選択を更新します。
...
区間判定に対応するmodel packは、同じプレーで信頼度の高い地点が繰り返される場合、それらを1候補へまとめます。信頼度が落ちた時間の長さ・候補間隔・全体の長さをクラスごとに評価して使い、別clipにはまたがりません。区間はモデルが反応した映像窓の範囲であり、厳密なプレー開始・終了時刻ではありません。区間判定はmodel pack内に保持し、Timelineの手動イベントを時間の近さだけで結合することはありません（[ADR 0036](adr/0036-event-episode-consolidation.md)）。

しきい値を区間判定の継続条件より低くした場合は、低信頼度の単独候補も表示され、重複が増えることがあります。旧モデルで追加済みのイベントは、再検出しても自動削除・結合されません。比較する場合は追加先の行名を変え、旧候補と区別して確認してください。
...
長いイベントの分割に対しては、Train映像の連続した場面を確認し、同一イベントの区間・別イベントへの切れ目・明示的な背景を記録します。AIによる確認も確認者と根拠を残した弱い教師データとして扱い、未確認区間を背景にしません。ニューラル重みの再学習と区間判定の調整は別に比較・報告します。区間判定でもCodingとの一致判定は代表時刻による既存方式を維持し、追加の確認済みフレームは実際に返す区間内に含まれるかを一対一で検証します。

確認済み映像による補正学習では、過去のTrain試合・同期した追加アングル・以前の確認済み例を引き継ぎます。既存の特徴抽出とイベント判定を保持し、過去の映像窓では判定の変化を抑え、確認済みの誤りには新しい補正重みを学習する方式も使用します。同じ特徴を再利用するため映像解析を二重に実行しません。補正後の区間判定も再評価し、過去に拾えた同一Codingイベント、追加の確認済み地点、別イベントの分離を保持します。学習した試合内の改善は未知試合の精度とは区別します（[ADR 0037](adr/0037-reviewed-model-refinement.md)）。

製品は完成したmodel packを使う構成を維持し、映像・Codingを暗黙にアップロードしません。学習とモデル比較の実装はprivate R&D側で管理します。

### 前後の映像と公開画像を使う補正

`reviewed-temporal`に対応する試験packは、2秒間隔の9地点（中心の前後8秒）の特徴を使い、リスタートとラインアウトを補正します。スクラムのスコア・しきい値・区間判定は基準packから保持します。文脈は同じ映像clip内に限定し、clipの端では端の特徴を繰り返します。既存の特徴を再利用するため、映像のデコードや特徴抽出を二重に実行しません。ランナーも新しい形式に対応したものをpackへ同梱します。

公開画像は、隊形の識別を補う学習例として比較できますが、キックやスローインの発生時刻を学習する動画ラベルとは区別します。調査した[CodeProcessorのデータセット](https://github.com/CodeProcessor/rugby-events-dataset)はスクラム・ラインアウト・ラックの画像を提供し、リスタートのクラスはありません。配布元のTrain画像375枚を使う候補と使わない候補を同条件で比較した結果、今回の`0.6.0-experimental.1`では画像を使わない候補を採用しました。公開データの追加自体を精度改善とみなしません。出典・ライセンス・固定revision・画像と特徴のhashはR&D側で確認・記録します。

このpackの開発比較では、別の2試合で基準packが検出していた同一の57/59 Codingイベントを維持し、一致しない候補はリスタート31→19、ラインアウト111→90、スクラム18→18となりました。一致しない候補には未記録の実イベントも含まれ得るため、誤検出数とは断定しません。別途目視確認したValidationの負例9地点では8地点に誤検出が残っています。改善が完了したモデルとは扱わず、試験表示・独立評価が必要な状態を維持します。
